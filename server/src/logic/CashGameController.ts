...
export class CashGameController implements IGameSession {
  ...
  constructor(options: CashGameControllerOptions) {
    ...
    this.turnTimeoutMs =
      ...
      options.turnTimeoutMs >= 3000 &&
      options.turnTimeoutMs <= 120_000
  ...
  private buildPlayersFromSeats(buttonFirst: boolean): Player[] {
    const occupied = this.seats
      ...
      .filter((s) => s.userId != null) as (CashSeat & {
  ...
  sit(
    ...
    if (this.gameTable != null)
      return { ok: false, error: "Une main est en cours" };
    if (seatIndex < 0 || seatIndex >= this.maxSeats)
      return { ok: false, error: "Siège invalide" };
    if (this.seats[seatIndex].userId != null)
      return { ok: false, error: "Siège occupé" };
    const amount = intChips(
      Math.max(this.defaultBuyIn, Math.min(buyIn, 10000)),
    );
  ...
  getSanitizedState(
    ...
    cashCountdownEndsAt?: number;
    cashSeats?: CashSeat[];
    spectatorRejoinQueue?: string[];
  } {
    const base = this.gameTable
      ? {
          ...this.gameTable.getSanitizedState(requestingPlayerId),
          cashCountdownEndsAt: undefined,
          cashSeats: this.seats,
        }
      : {
          ...this.state,
          phase: this.countdownEndsAt ? "WAITING" : ("WAITING" as const),
        };
  ...
  async processRejoinQueue(
    getUser: (
      userId: string,
    ) => Promise<{ username: string; chips: number } | null>,
  ): Promise<void> {
    ...
    for (const userId of toProcess) {
      ...
      if (free >= 0) {
        this.sit(
          ...
          user.username,
          free,
          Math.max(this.defaultBuyIn, user.chips),
        );
      } else {
