...
router.post("/start", async (req, res) => {
  try {
    ...
    if (waitingRoom.hostId !== hostId) {
      return res
        .status(403)
        .json({ error: "Seul le host peut démarrer la partie" });
    }
...
router.get("/:gameId", (req, res) => {
  ...
  console.log(
    `🔍 Recherche de la partie: ${gameId}. Clés dans le Map:`,
    Array.from(activeGames.keys()),
  );
...
// GET /api/game/:gameId/history - Historique d'une partie
router.get("/:gameId/history", async (req, res) => {
  try {
    ...
    return res.json({
      ...
      date: result?.createdAt ?? result?.endedAt ?? null,
    });
  } catch (error) {
