...
\subsection{Planning and Gantt evolution}

The initial Gantt chart separated specification, architecture, implementation, testing and reporting in a mostly sequential way. This matched the early V-model framing, but the actual project quickly became more iterative: poker runtime work, UI integration, WebSocket behavior, database migrations and deployment constraints had to be tested together rather than at the end. The planning therefore evolved into shorter delivery cycles: first a playable poker baseline, then real-time stabilization, then casino/tournament/social extensions, then security hardening, CI/CD and final documentation.

The main planning adjustment was to move high-risk technical work earlier. Redis integration, Socket.IO rooms, Prisma schema changes, recovery services, security middleware and CI fixes were treated as blocking foundations, while visual polish and secondary flows were scheduled after the core runtime became stable. This reduced late surprises and made it possible to keep a demonstrable product during the final weeks.
...
\subsection{Response to human and coordination difficulties}

The team faced typical project constraints: uneven availability, changing technical confidence across members, merge conflicts, and moments where one feature depended on another person's backend, frontend or database work. The response was pragmatic rather than formal: risky tasks were discussed in meetings, integration problems were moved into GitLab merge requests, and members with more context helped unblock others through reviews, pair debugging and shared documentation. When a strict role split became inefficient, the team accepted cross-functional work so that security, real-time behavior, UI fixes and deployment could progress together.

This flexibility was especially important near the deadline. Instead of freezing the project after the first complete version, the team continued to correct pipeline failures, merge conflicts, report inconsistencies and runtime bugs. The final organization was therefore closer to a controlled agile workflow than to the initial linear plan: ownership remained visible, but the priority was to keep the product buildable, testable and explainable.
...
\node[flow, fill=blue!5] (n0) {\textbf{1. Input} -- JSON payload to \texttt{/predict/poker} (visible cards, pot, \texttt{toCall}, stacks, street, short history).};
\node[flow, fill=blue!10, below=4mm of n0] (n1) {\textbf{2. Feature vector} -- \texttt{build\_features} $\rightarrow$ vector $\mathbf{x}$ with \textbf{36} values (\texttt{FEATURE\_NAMES}).};
...
\node[flow, fill=HeadingRule!16, below=4mm of n4] (n5) {\textbf{6. Output head} -- logits $\boldsymbol{\ell}=W_4\mathbf{h}_3+\mathbf{b}_4 \in \mathbb{R}^{4}$ aligned with FOLD / CALL\_CHECK / RAISE / ALL\_IN.};
\node[flow, fill=green!10, below=4mm of n5] (n6) {\textbf{7. Decision in \texttt{decision.py}} -- teacher/heuristic score mixing, softmax, temperature sampling, legal amount conversion.};
...
\end{tikzpicture}
\caption{Data flow through the expert MLP prototype (\texttt{ExpertPolicy}): 36-feature vector, dense layers, then domain-specific post-processing before the HTTP response.}\label{fig:ai-mlp}
\end{figure}
...
\node[blk, fill=gray!8, text width=0.88\textwidth] (start) {\textbf{Start} -- bot turn, \texttt{expert} difficulty (\texttt{POST /api/bot/action}).};
\node[dia, below=5mm of start] (durl) {\textbf{\texttt{AI\_SERVICE\_URL}} set?};
\node[blk, fill=orange!15, below left=6mm and 2mm of durl] (tsfb) {\textbf{No branch} -- \texttt{expertBotDecision} (pure TypeScript, no trainable neuron).};
...
\node[blk, fill=gray!10, below=12mm of durl |- tsfb2.south] (end) {\textbf{End} -- action applied by the authoritative server; \texttt{expert\_ai\_decision} logged when Python is used.};
...
\draw[->, thick] (zod) |- (end);
\end{tikzpicture}
\caption{Node-side decision flow for the \texttt{expert} tier: \texttt{botAi.service.ts} orchestration, heuristic fallback and response hardening.}\label{fig:ai-flow}
\end{figure}
...
\node[archstep, fill=blue!10, below=4mm of c] (a) {\textbf{2. Transports} -- REST \texttt{/api/*} (JWT) and Socket.IO \texttt{/socket.io} (same token model).};
\node[archstep, fill=blue!14, below=4mm of a] (g) {\textbf{3. Node edge} -- Express middlewares/rate limits + \texttt{GameGateway} real-time events and timers.};
\node[archstep, fill=blue!18, below=4mm of g] (s) {\textbf{4. Domain services} -- poker, blackjack, tournaments, casino, friends, progression, etc.};
\node[archstep, fill=HeadingRule!14, below=4mm of s] (p) {\textbf{5. Persistence \& coordination} -- Prisma $\rightarrow$ PostgreSQL; Redis for rate limits, Socket.IO adapter and logs.};
\draw[->, thick] (c) -- (a);
\draw[->, thick] (a) -- (g);
\draw[->, thick] (g) -- (s);
\draw[->, thick] (s) -- (p);
\end{tikzpicture}
\caption{Monorepo execution flow: user interface $\rightarrow$ API / WebSocket $\rightarrow$ backend edge and services $\rightarrow$ data layer.}\label{fig:flow}
\end{figure}
...
The final implementation is best understood as a modular real-time gaming platform, not as a single poker screen with extra pages attached. React handles the user interface, Express exposes REST APIs, Socket.IO delivers live events, Prisma/PostgreSQL stores durable state, and Redis supports coordination, rate limits, token revocation and distributed sockets.

The main design decision is server authority. Poker, blackjack, roulette, slots, tournaments, hidden bets, rewards and social money flows are validated and settled on the server; the client displays state and sends user intentions. Chip-changing actions use persistence and ledger-oriented patterns so the database remains the source of truth. The same principle is applied to private poker cards, blocked-user warnings, invitations, reports, XP, badges, daily challenges and leaderboard updates.
...
Validation and tests were added around the most sensitive areas: authentication schemas, trusted waiting-room identity, socket authentication, runtime snapshots, game logic and tournament utilities. The body keeps the project readable; the appendices act as the technical evidence trail: Appendices~\ref{app:map}, \ref{app:poker}, \ref{app:realtime}, \ref{app:social}, \ref{app:prog}, \ref{app:rewards}, and~\ref{app:coverage}.
...
Table~\ref{tab:future-work} gives a quantified view of realistic improvements beyond the academic prototype. Estimates assume one developer already familiar with the codebase; parallel work could shorten calendar time but would still require review and integration.

\begin{table}[htbp]
\centering
\small
\begin{tabular}{@{}p{4.4cm}p{6.6cm}p{2.1cm}@{}}
\toprule
Improvement & Scope & Estimated effort \\
\midrule
Production security review & Threat model, secret rotation policy, dependency review, stricter deployment variables, manual abuse scenarios & 1--2 weeks \\
Observability dashboard & Structured metrics dashboards, alert rules, WebSocket health panels, Redis/PostgreSQL runtime indicators & 1 week \\
Full end-to-end test campaign & Playwright coverage for lobby-to-game flows, invitations, tournaments, blocked-user warnings and recovery paths & 2 weeks \\
External identity/email providers & Real email delivery, account verification, password reset hardening, provider configuration and privacy review & 2--3 weeks \\
Certified payment/banking integration & Replace simulated top-ups with a compliant provider, audit trails, legal review and failure handling & 4--6 weeks \\
Advanced bot evaluation & Controlled self-play dataset, evaluation metrics, comparison against heuristics, clearer model documentation & 2--3 weeks \\
Tournament analytics & Post-tournament statistics, player history, richer admin insights and exportable summaries & 1--2 weeks \\
\bottomrule
\end{tabular}
\caption{Quantified future work estimates for a production-oriented continuation.}\label{tab:future-work}
\end{table}

The highest-priority continuation would be security review and broader end-to-end testing, because these reduce operational risk before adding new features. Certified identity or payment flows should only be considered if the project leaves its academic, virtual-chip context.
