...
\subsection{UI screenshot placeholders}
\label{subsec:ui-screenshot-placeholders}

The following placeholders reserve space for final interface screenshots. Each capture should be taken from the current deployed or demo-ready build, preferably with realistic but non-sensitive test data. The goal is to show the user-facing result of the platform, not internal implementation details.

\begin{figure}[htbp]
\centering
\fbox{\begin{minipage}[c][3.8cm][c]{0.88\textwidth}
\centering
\textbf{Poker table UI screenshot placeholder}\\[0.4em]
Capture an active Texas Hold'em table with community cards, player seats, current chips, action buttons, pot area and the Quantum HUD visible.
\end{minipage}}
\caption{Placeholder for the poker table interface screenshot.}
\label{fig:ui-poker-placeholder}
\end{figure}
...
\fbox{\begin{minipage}[c][3.8cm][c]{0.88\textwidth}
\centering
\textbf{Main lobby UI screenshot placeholder}\\[0.4em]
Capture the main navigation hub showing access to poker, casino games, tournaments, social features, rewards and profile/progression areas.
\end{minipage}}
\caption{Placeholder for the main lobby interface screenshot.}
\label{fig:ui-lobby-placeholder}
\end{figure}
...
\fbox{\begin{minipage}[c][3.8cm][c]{0.88\textwidth}
\centering
\textbf{Tournament UI screenshot placeholder}\\[0.4em]
Capture either the tournament waiting room, an active tournament table, or the tournament results screen with bracket/progression information visible.
\end{minipage}}
\caption{Placeholder for the tournament interface screenshot.}
\label{fig:ui-tournament-placeholder}
\end{figure}
...
\fbox{\begin{minipage}[c][3.8cm][c]{0.88\textwidth}
\centering
\textbf{Blackjack UI screenshot placeholder}\\[0.4em]
Capture the blackjack table with player hand, dealer hand, bet controls, action buttons and displayed result or current round state.
\end{minipage}}
\caption{Placeholder for the blackjack interface screenshot.}
\label{fig:ui-blackjack-placeholder}
\end{figure}
...
\fbox{\begin{minipage}[c][3.8cm][c]{0.88\textwidth}
\centering
\textbf{Leaderboard UI screenshot placeholder}\\[0.4em]
Capture the leaderboard or progression view with rankings, XP/level information, badges or player statistics visible.
\end{minipage}}
\caption{Placeholder for the leaderboard and progression interface screenshot.}
\label{fig:ui-leaderboard-placeholder}
\end{figure}
...
\fbox{\begin{minipage}[c][3.8cm][c]{0.88\textwidth}
\centering
\textbf{Admin panel UI screenshot placeholder}\\[0.4em]
Capture the protected admin console with moderation, reports, monitoring-oriented information or gift-code management visible. Use test data only.
\end{minipage}}
\caption{Placeholder for the admin panel interface screenshot.}
\label{fig:ui-admin-placeholder}
\end{figure}
