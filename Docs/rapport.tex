...
The early scope targeted: Texas Hold'em cash games; \textbf{AI practice} with four difficulty tiers (\texttt{easy}, \texttt{medium}, \texttt{hard}, \texttt{expert}); public/private multiplayer rooms with SB/BB configuration and minimum balance rules; \textbf{real-time} updates via Socket.IO; a \textbf{Quantum HUD} for win-equity and hand-category feedback; and \textbf{hidden bets} resolved server-side. See Appendices~\ref{app:poker}, \ref{app:bots}, \ref{app:hud}, \ref{app:hidden}, and~\ref{app:realtime}.
...
\node[blk, fill=gray!10, below=12mm of durl |- tsfb2.south] (end) {\textbf{End} — action applied by the authoritative server; \texttt{expert\_ai\_decision} logged when the Python service is used.};
...
\caption{Node-side decision flow for the \texttt{expert} bot tier: optional Python service call, heuristic fallback, validation and authoritative action application.}\label{fig:ai-flow}
\end{figure}
...
The rewards and wallet system orchestrates chip circulation, XP progression, and peer-to-peer lending across all game modes: poker cash games, casino games, tournaments, and social loans. All transactions are recorded immutably in a \textbf{wallet ledger} with cryptographic integrity hashing (SHA-256), providing audit-style traceability for the virtual economy. The system enforces \textbf{server-side authority}---clients never compute balance changes; they request actions and receive confirmed outcomes via WebSocket. Every chip movement is atomic: ledger entry and balance update commit together via Prisma transactions, or neither commits. See Appendix~\ref{app:rewards} for ledger fields and settlement examples.
...
Prisma models capture users, games, tournaments, loans, hidden bet tickets, wallet ledger rows, and more. Critical monetary flows favor server-side orchestration and short transactions. Appendix~\ref{app:rewards} explains the ledger side of these models.
...
\draw[->, thick] (s) -- (p);
\end{tikzpicture}
\caption{Monorepo runtime flow: interface $\rightarrow$ API / WebSocket $\rightarrow$ gateway and services $\rightarrow$ data layer.}\label{fig:flow}
\end{figure}
...
Redis-backed Socket.IO adapter, structured logging, Prometheus metrics endpoint, readiness endpoints and graceful shutdown hooks support stability. See Appendices~\ref{app:realtime} and~\ref{app:ops}.
...
The \texttt{docker-build} stage invokes \textbf{Kaniko} executor \texttt{v1.23.2-debug} with an empty container \texttt{entrypoint} so the shell can call \texttt{/kaniko/executor}. \textbf{(1)}~\texttt{docker:build}---\path{Dockerfile}, context \path{server}, destinations \texttt{\$\{CI\_REGISTRY\_IMAGE\}:\$\{CI\_COMMIT\_SHORT\_SHA\}} and \texttt{:latest}; runs on \texttt{develop}, \texttt{main}, and \texttt{release/*}. \textbf{(2)}~\texttt{docker:build:client}---\path{Dockerfile}, build-arg \texttt{VITE\_API\_URL} (override in GitLab CI/CD variables for behind-nginx paths), \texttt{KUBERNETES\_MEMORY\_REQUEST}/\texttt{LIMIT} raised to reduce OOM during Kaniko layer snapshots; destinations \texttt{:client-\$\{CI\_COMMIT\_SHORT\_SHA\}} and \texttt{:client-latest}; same branch rules as~(1). \textbf{(3)}~\texttt{docker:build:ai-service}---context \path{ai-service}, FastAPI stack for the optional poker prediction endpoint; destinations \texttt{:ai-\$\{CI\_COMMIT\_SHORT\_SHA\}} and \texttt{:ai-latest}; \textbf{only} \texttt{release/*} (commented in YAML to spare runner disk on \texttt{develop}/\texttt{main}). All three jobs use \texttt{--compressed-caching=false} for predictable memory on small Kubernetes runners. \path{Dockerfile} documents Kaniko-oriented Dockerfile layering (grouped \texttt{RUN} steps) to avoid oversized intermediate snapshots after \texttt{npm ci}.
...
Poker actions are routed through \path{pokerActionOrchestrator.service.ts} and the gateway; table controllers under \path{logic} keep the authoritative model. Hidden bets are separated under \path{hiddenBets}, which lets side markets be priced, placed and resolved without leaking private poker information to the browser.

Synchronization combines room-based events, full snapshots and recovery stores. Important table updates are broadcast to Socket.IO rooms, while reconnecting clients receive complete sanitized snapshots. Poker runtime snapshots are serialized through \path{pokerStateSync.service.ts} and stored behind \path{pokerStateStore.ts}; blackjack has its own snapshot repository and boot-time recovery service.

\subsection{Extended modules}

Casino games are isolated behind their own routes and services instead of being mixed into the poker engine. Slot and roulette use dedicated logic modules, payout calculation, stake caps and wallet updates. Blackjack exists in solo and multiplayer forms, with table rooms, seats, snapshots and recovery services under \path{blackjack}.

Tournaments separate registration, waiting state, table assignment, runtime progression, finalization and rewards. Tournament screens live under \path{tournament}; server-side orchestration lives under \path{tournament}. Winner side bets are implemented as a tournament service so settlement remains tied to the real tournament result.
