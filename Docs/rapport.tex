...
The repository does not contain signed-off, per-person effort logs or story point assignments for every teammate. The table below therefore combines the expanded narratives above with repository ownership notes such as \path{CODEOWNERS}; it should still be countersigned by the team before final submission.
...
\midrule
Linda & Security, network, deployment safeguards, CI/CD review & \path{CODEOWNERS}: security/network/docker/config/.gitlab ownership; critical files and CI/CD review paths \\
Abdoulaye & Database modeling, Prisma schema, migrations, SQL/database validation & \path{CODEOWNERS}: database, migrations, SQL, Prisma schema and database test ownership \\
Soheil & Backend logic, services, tests, poker evaluation support & \path{CODEOWNERS}: backend/API/services ownership; references in evaluator and backend test fixtures \\
Mohamed & Frontend technical lead, React pages/components, architecture coordination & \path{CODEOWNERS}: frontend/components/pages ownership; architecture and critical file review paths \\
...
\bottomrule
\end{tabular}
\caption{Individual contributions: expanded statements for Omar, Massi, Yigit and Azra appear as subsections above; other rows are summarized from repository ownership notes and should be countersigned by the team.}\label{tab:contrib}
\end{table}
