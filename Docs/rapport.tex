...
When \texttt{AI\_SERVICE\_URL} points to the FastAPI bundle under \path{server/ai-service/}, the expert path can run a \textbf{small supervised MLP prototype} (\texttt{ExpertPolicy} in \path{server/ai-service/poker/train.py}, weights serialised to \path{server/ai-service/model/expert_bot.pt}). This is not presented as a research-grade poker solver or reinforcement-learning agent. Its role is narrower: it experiments with a learned approximation of the handcrafted expert teacher while keeping a deterministic TypeScript fallback available at all times.

The vectoriser \path{features.py} exports \texttt{FEATURE\_NAMES}: \textbf{36} numeric descriptors (streets, equity, pot odds, SPR, board texture, action frequencies, etc.). The network stacks \textbf{four linear layers} separated by ReLU: $36\to 96 \to 64 \to 32 \to 4$ logits. Counting post-input activation units (hidden $+$ output heads) gives \textbf{$96+64+32+4=196$ neurons}. Dropout ($p{=}0.06$ then $0.04$) is applied after the first two hidden blocks during training only. The four outputs correspond to internal action labels produced by the expert-rule teacher (\path{expert_rules.py}); inference-time mixing with heuristics and temperature sampling is described in \path{server/ai-service/poker/decision.py}. The compact trainable weight count in the linear stack is \textbf{11\,972} scalar parameters (weights plus biases). Figures~\ref{fig:ai-mlp} and~\ref{fig:ai-flow} show the tensor pipeline and the Node$\leftrightarrow$Python decision path. Appendix~\ref{app:bots} contains the heuristic fallback and dispatch details.
...
This architecture keeps outcome and ledger updates tied to the same server transaction: in normal operation they commit together, or neither commit. Clients cannot force outcomes; they can only place bets and observe server decisions. The same transaction pattern is expanded in Appendix~\ref{app:rewards}.
...
Synchronization is based on rooms and snapshots. Each user has a private \texttt{user:\{userId\}} room, while poker tables, blackjack tables and tournaments use dedicated rooms for shared updates. On join or reconnect, the server sends a complete sanitized snapshot instead of relying only on incremental events. This makes reconnection easier for the player and avoids rebuilding a game from stale partial events.

Confidentiality is also handled on the server. A seated player receives their own private cards, while other players and spectators receive a sanitized view. Redis support allows Socket.IO events to work across several server instances. The full event model, table locking, reconnection behavior and security checks are detailed in Appendix~\ref{app:realtime}.
...
\toprule
Target & Main command & Generated artefact & Main repository files \\
\midrule
Web & \texttt{npm run build} & Static Vite build in \path{client/dist/} & \path{client/src/}, \path{client/vite.config.ts} \\
Windows desktop & \texttt{npm run electron:build:win} & NSIS \texttt{.exe} installer & \path{client/electron.cjs}, \path{client/package.json} \\
macOS desktop & \texttt{npm run electron:build:mac} & macOS app / \texttt{.dmg} & \path{client/electron.cjs}, \path{client/package.json} \\
Linux desktop & \texttt{npm run electron:build:linux} & \texttt{AppImage} and \texttt{.deb} & \path{client/electron.cjs}, \path{client/package.json} \\
Android & \texttt{npm run cap:android} & Android Studio project / APK via Gradle & \path{client/capacitor.config.ts}, \path{client/android/} \\
iOS & \texttt{npm run cap:ios} & Xcode project / archive & \path{client/capacitor.config.ts}, \path{client/ios/} \\
\bottomrule
\end{tabular}
...
The project was completed by decomposing the platform into domain modules while keeping one consistent runtime architecture: React for presentation, Express for REST APIs, Socket.IO for live state, Prisma/PostgreSQL for durable data, and Redis for coordination. This avoids a single monolithic ``game page'' and lets poker, blackjack, casino mini-games, tournaments, social features and progression evolve under the same server-authoritative model. The module map is in Appendix~\ref{app:map}.
...
The same pattern applies to the extended modules. Casino games are isolated behind their own service layers; tournaments separate registration, table assignment, progression and reward settlement; social features combine REST persistence with socket notifications; progression and daily rewards remain server-validated. Security is deliberately layered across HTTP middleware, socket authentication, gateway checks, rate limits, idempotency and anti-cheat alerts. Monetary integrity is enforced through Prisma transactions and immutable wallet ledger entries. Further implementation detail is moved to Appendix~\ref{app:implemented-solutions}.
...
Organizational narrative: Section~\ref{sec:management}. Beyond meetings and backlog arbitration, I coordinated \textbf{cross-cutting technical commitments}: early adoption of Redis for Socket.IO horizontal scale-out and rate limiting, alignment between Prisma schema changes and gameplay features, and risk reviews before widening scope (tournaments, casino mini-games, loans). I also worked with the appointed technical lead to sequence risky merges, such as the socket gateway and recovery jobs, away from demo deadlines.

I also supervised the quality objective around automated test coverage. My role was to keep the backend coverage target as close as possible to the 80\% threshold expected by the project, identify weakly covered modules, and make sure tests were added before final validation. This effort succeeded in bringing the measured server coverage above the required baseline, as reported in Section~\ref{sec:security} and Appendix~\ref{app:coverage}.
...
I had a transversal role across backend integration, real-time features, security hardening and UI/UX. On the backend side, I worked on game routes, blinds configuration, waiting-room fixes, player-state debugging and several security-critical paths: socket authentication, JWT hardening, stronger password rules, suspended-user checks, rate limits, admin route protection and anti-cheat alerts.

I also implemented and stabilized a large part of the social experience. This included friend search, add-friend flows, friend cards, chat access from the lobby, remove friend, block/unblock, user reports, invitation handling, live notifications, activity status and warnings when joining rooms containing blocked users. These changes required both REST API work and Socket.IO behavior, because the social features had to remain live without page refreshes.

On the frontend side, I contributed heavily to the final user experience: lobby layout, desktop fit, removal of unnecessary scrollbars, profile and avatar behavior, poker table layout, blackjack, roulette, mini-games, settings, audio controls and responsive fixes. I also helped with debugging, merge conflict resolution, CI pipeline fixes, PDF/report finalization and repeated end-to-end validation before the final delivery. Overall, my work was mostly cross-functional: connecting backend behavior, real-time synchronization, security rules and interface polish into features that users could actually use.

\subsection{Mohamed -- Technical Leader}

I worked as the \textbf{Technical Leader}. My role mainly consisted of coordinating the development of several core systems and contributing to both the frontend and backend parts of the platform in order to maintain a stable and consistent multiplayer experience.

I contributed to the finalization of the user interface and game design by improving responsiveness across desktop and Android platforms. I also worked on multiplayer animations, round transitions, and automatic pot calculations to improve gameplay fluidity and synchronization. In parallel, I participated in the visual design preparation of future mini-games such as the Slot Machine and Roulette through Figma prototypes.

I contributed to defining the tournament rules and implementing the tournament system, while also working with the project leader Omar on fixing bugs and improving system stability during testing phases.

On the security side, I worked on anti-cheat and moderation mechanisms, including multi-account detection based on IP analysis and heuristic bot detection using player response times. I also contributed to the implementation of a global banning middleware and secured administration API routes used for sanction monitoring and moderation tasks.

I additionally contributed to infrastructure stability and production deployment by configuring the production virtual machine, improving API protection through stricter CORS policies and rate limiting, and connecting the CI/CD pipeline to automated deployment workflows. I also participated in the debugging phase by identifying and fixing critical issues related to gameplay stability and multiplayer synchronization.

Finally, I participated in tournament playtesting sessions under real conditions and contributed to player retention systems such as daily rewards and cooldown mechanisms.

\subsection{Abdoulaye -- Database Architecture, Backend Integration and Stabilization}

I mainly contributed to the database architecture, backend integration and cross-platform stabilization of Quantum Bluff. My first responsibilities focused on the design and maintenance of the database layer using PostgreSQL and Prisma ORM. I worked on schema evolution, migrations, indexing and data persistence so that gameplay, social features and user progression could rely on consistent relational data.

I also contributed to the ERD database schema, waiting-room models, friend-system relations, statistics management and game-history structures. Several optimization and integrity tasks were carried out to improve query efficiency and maintain consistency between users, rooms and gameplay records. This work helped keep the persistence layer aligned with the multiplayer logic and the reporting needs of the platform.

I participated in infrastructure and deployment preparation by working on Prisma configuration, Docker database setup, backup scripts and seed generation for development and testing environments. I also contributed to discussions around separating authentication and game databases in order to improve security, maintainability and system resilience.

On the frontend and user-experience side, I contributed to interface flows including login and registration pages, poker table integration, lobby navigation, accessibility settings, audio configuration and table customization. I also participated in usability feedback and testing by identifying UI issues, accessibility limitations and gameplay synchronization problems during multiplayer sessions.

Beyond technical implementation, I regularly contributed to project coordination through task follow-up, documentation updates, reporting and testing organization. I helped monitor stabilization progress during the beta phase and participated in debugging and validation efforts across multiple platforms, including web, Electron and Capacitor builds. Overall, my contribution combined database engineering, backend support, testing, deployment preparation and interface integration to improve the stability and maintainability of the Quantum Bluff platform.

\subsection{Soheil -- Backend Support and Testing}

I contributed to the project mainly through backend support, code review and testing activities. My work focused on helping validate service behavior, checking backend logic during integration phases, and participating in debugging when issues appeared around gameplay flows or API interactions.

I also contributed to the quality assurance effort by reviewing selected backend changes, helping identify regressions, and supporting the team during final stabilization. My contribution was more limited in scope than the main feature owners, but it helped reinforce the reliability of the backend and test validation process before delivery.
...
The next improvements are intentionally scoped as measurable increments rather than open-ended wishes:

\begin{itemize}
\item \textbf{End-to-end testing:} add 8--10 Playwright scenarios covering the full lobby-to-game journey (login, create room, join poker table, create tournament, play blackjack, claim reward), targeting roughly \textbf{+15\% additional critical-flow coverage} within \textbf{2 weeks}.
\item \textbf{Recovery and observability:} add a Grafana-style dashboard for Socket.IO rooms, failed actions, tournament recovery and wallet mutations, targeting recovery diagnosis in \textbf{under 5 minutes} and reducing manual log inspection during incidents.
\item \textbf{Real-time scaling:} validate a \textbf{3-node Redis-backed Socket.IO deployment} with sticky sessions and room broadcasts, then compare reconnection time against the current single-instance setup.
\item \textbf{Tournament analytics:} add tournament-level KPIs (average round duration, bust-out order, table wait time, winner-bet volume) within \textbf{1--2 weeks}, starting from the existing tournament tables and ledger events.
\item \textbf{Certified identity/payment providers:} if the project leaves the academic prototype context, replace simulated top-ups and internal verification with certified providers for Google verification, OTP delivery and banking/payment flows. This depends on external compliance approval rather than only development time.
\end{itemize}
...
The implemented solution decomposes the product into modules while preserving a shared runtime architecture. The React client handles navigation, presentation, forms, table interaction and user feedback. The Express API exposes stable REST endpoints. Socket.IO carries live events. Prisma/PostgreSQL stores durable state, and Redis supports coordination, rate limiting, token revocation and distributed socket delivery.

\subsection{Authoritative gameplay and synchronization}

Poker actions are routed through \path{server/src/poker/services/pokerActionOrchestrator.service.ts} and the gateway; table controllers under \path{server/src/logic/} keep the authoritative model. Hidden bets are separated under \path{server/src/poker/hiddenBets/}, which lets side markets be priced, placed and resolved without leaking private poker information to the browser.

Synchronization combines room-based events, full snapshots and recovery stores. Important table updates are broadcast to Socket.IO rooms, while reconnecting clients receive complete sanitized snapshots. Poker runtime snapshots are serialized through \path{server/src/poker/services/pokerStateSync.service.ts} and stored behind \path{server/src/shared/pokerStateStore.ts}; blackjack has its own snapshot repository and boot-time recovery service.

\subsection{Extended modules}

Casino games are isolated behind their own routes and services instead of being mixed into the poker engine. Slot and roulette use dedicated logic modules, payout calculation, stake caps and wallet updates. Blackjack exists in solo and multiplayer forms, with table rooms, seats, snapshots and recovery services under \path{server/src/blackjack/}.

Tournaments separate registration, waiting state, table assignment, runtime progression, finalization and rewards. Tournament screens live under \path{client/src/features/tournament/}; server-side orchestration lives under \path{server/src/tournament/}. Winner side bets are implemented as a tournament service so settlement remains tied to the real tournament result.
...
Security is redundant by design: HTTP middleware, socket middleware, gateway checks and domain services re-check sensitive assumptions in their own layer. Forged REST bodies cannot replace JWT identity, forged socket payloads cannot act for another player, and suspended accounts are rejected at login, REST middleware and socket connection time.

Chip-changing operations use Prisma transactions, wallet ledger entries, idempotency keys and domain-specific settlement services. This pattern appears in casino modules, hidden-bet placement/resolution, tournament rewards and social loan flows. Client-side balances are treated as cached UI; the database and server services remain the source of truth.
...
\label{subsec:user-workflow-placeholder}

Figure~\ref{fig:user-workflow-placeholder} is reserved for a compact visual summary of the main user journey. It should show the path from authentication to the lobby, then the three principal branches: gameplay, social interaction, and progression/rewards. The purpose of this diagram is not to duplicate the technical architecture, but to give a non-technical reader a quick understanding of how a player moves through Quantum Bluff.
...
\begin{tikzpicture}[
  font=\footnotesize,
  >=Stealth,
  wfstep/.style={rectangle, draw=HeadingRule, rounded corners=2mm, align=center, minimum width=2.5cm, minimum height=8mm, fill=blue!5},
  branch/.style={rectangle, draw=black!55, rounded corners=2mm, align=center, minimum width=2.8cm, minimum height=8mm, fill=gray!8},
  note/.style={rectangle, draw=orange!70!black, dashed, rounded corners=2mm, align=center, minimum width=9.5cm, minimum height=7mm, fill=orange!6}
]
\node[wfstep] (auth) {Login / Register};
\node[wfstep, right=8mm of auth] (lobby) {Main lobby};
\node[branch, below left=8mm and 0mm of lobby] (game) {Play\\poker, casino\\tournaments};
\node[branch, below=8mm of lobby] (social) {Interact\\friends, invites\\loans};
\node[branch, below right=8mm and 0mm of lobby] (progress) {Progress\\XP, rewards\\leaderboard};
\node[note, below=10mm of social] (placeholder) {Placeholder: replace or refine with the final rendered user workflow diagram before submission.};
\draw[->, thick] (auth) -- (lobby);
\draw[->, thick] (lobby) -- (game);
\draw[->, thick] (lobby) -- (social);
\draw[->, thick] (lobby) -- (progress);
\draw[->, dashed] (game) -- (placeholder);
\draw[->, dashed] (social) -- (placeholder);
\draw[->, dashed] (progress) -- (placeholder);
\end{tikzpicture}
\caption{Placeholder mini user workflow: authentication, lobby entry, gameplay, social interaction and progression/reward loops.}
\label{fig:user-workflow-placeholder}
\end{figure}
