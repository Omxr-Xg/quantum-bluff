...
The baseline expectations for the Bachelor project are captured in the repository specification \path{Docs/Cahier_de_charge_6A_PI.pdf}. The delivered system \textbf{extends} that poker-centric charter: all original poker-facing commitments (cash tables, bots, HUD, hidden bets, real-time play) remain grounded in server authority, while additional modules (casino mini-games, tournaments, social lending, progression) were integrated under the same engineering constraints (tests, rate limits, transactional chip movements). Where the PDF and the code diverge, \textbf{the implementation and this report defer to the repository as the operational source of truth}, with internal notes under \path{Docs/detail_logique/}, the repository map in Appendix~\ref{app:map}, and the relational database overview in Appendix~\ref{app:db} providing rationale.
...
\textbf{Client:} React 19, TypeScript, Vite 7, Tailwind CSS, Radix UI primitives (\path{client/src/components/ui/}), react-router-dom v7, Redux Toolkit with RTK Query (\path{client/src/services/api.ts}), socket.io-client (\path{client/src/services/socket.ts}), i18next (\path{client/src/i18n/config.ts}). Route and provider mapping is summarized in Appendix~\ref{app:map}.

\textbf{Server:} Node.js (ESM), Express 4, Socket.IO 4, Prisma 6 with PostgreSQL (\path{server/prisma/schema.prisma}), Redis clients for rate limiting, action logging, and the Socket.IO Redis adapter (\path{server/src/index.ts}), Zod validation, JWT auth, bcrypt password hashing, sanitize-html where applicable. Socket and authentication details are expanded in Appendices~\ref{app:realtime} and~\ref{app:auth}.

\subsection{Database and infrastructure}

Local PostgreSQL via \path{database/docker-compose.yml}. Redis for multi-instance coordination and abuse controls. Optional OpenTelemetry and Prometheus-oriented metrics endpoints (\path{server/src/observability/}). Operational setup is detailed in Appendix~\ref{app:ops}.

\subsection{Version control and collaboration}

Git repository with GitLab CI configuration at \path{.gitlab-ci.yml}. Beyond unit tests and security scans, the \texttt{docker-build} stage uses \textbf{Kaniko} (\texttt{gcr.io/kaniko-project/executor:v1.23.2-debug}) to build and push OCI images \textbf{without a Docker daemon} on GitLab runners: jobs \texttt{docker:build} (server), \texttt{docker:build:client} (Vite frontend with \texttt{VITE\_API\_URL} build-arg), and \texttt{docker:build:ai-service} publish to the project's GitLab Container Registry for VM pull-based deployment. Documentation and logic notes live under \path{Docs/} (including \path{docs/ARCHITECTURE.md} and \path{Docs/detail_logique/}); CI and packaging details are in Appendix~\ref{app:ops}.
...
Table~\ref{tab:inventory} lists major domains. Details appear only once, in the referenced appendix, to prevent duplication. \textbf{Appendices~\ref{app:map}--\ref{app:sources}} also record implemented formulas and structures: relational database model, closed-form \textbf{slot RTP} and unified \textbf{roulette RTP}, Monte Carlo equity with an error bound for the \textbf{Quantum HUD}, \textbf{Quantum Combi} exact probability plus its place in the hand-ranking cascade, \textbf{tournament bracket} sizing and RNG, \textbf{code coverage} definitions, and the source files where each is coded.
...
Poker is implemented as an \textbf{authoritative server game}: the client sends intentions, while the server validates turns, blinds, raises, folds, pots, side pots and showdown results. The tournament layer reuses this poker engine and adds registration, entry fees, seeded bracket generation, table assignment, elimination tracking, final rewards and winner side bets. The full poker engine and tournament lifecycle are detailed in Appendices~\ref{app:poker} and~\ref{app:tournament}.

\begin{itemize}
\item \textbf{Poker core}: \path{GameTable.ts} and \path{CashGameController.ts} enforce legal actions and chip movement.
\item \textbf{Tournament orchestration}: \path{server/src/tournament/} creates rounds, tables and transitions.
\item \textbf{Real time}: Socket.IO rooms notify players about table assignment, next rounds, eliminations and completion.
...
Entry fees, rewards and winner-bet settlements run inside Prisma transactions. Action deduplication (\texttt{handId + actionId}) prevents duplicate mutations after packet retransmission. Game snapshots are persisted after actions and can be restored after restart. Table sizes adapt to player count (for example $[4,4]$ for 8 players and $[5,4]$ for 9 players). Appendix~\ref{app:tournament} gives the bracket arithmetic, RNG seeding and tournament flow diagram.
...
This architecture guarantees consistency: outcome and ledger entry commit together, or neither commits. Clients cannot force outcomes; they can only place bets and observe server decisions. The same transaction pattern is expanded in Appendix~\ref{app:rewards}.
...
Daily challenges and login streaks (separate modules in \path{server/src/dailyChallenges/}, \path{server/src/dailyLogin/}) award XP using the same \texttt{awardXp} function, feeding into the gamification bundle. Client displays these in \path{/profile} and \path{/leaderboard} routes. Appendix~\ref{app:prog} links the relevant modules.
...
This ensures no information leakage: folded hands remain hidden, private cards are visible only to their owner, and the client cannot craft a request to peek at other hands. This role-based socket view is part of the real-time architecture in Appendix~\ref{app:realtime}.
...
Prisma models capture users, games, tournaments, loans, hidden bet tickets, wallet ledger rows, and more (see \path{server/prisma/schema.prisma}). Critical monetary flows favor server-side orchestration and short transactions. Appendix~\ref{app:rewards} explains the ledger side of these models.
...
Figure~\ref{fig:flow} shows a \textbf{runtime flow diagram} (logical path, not deployment topology). For more detail, see \path{docs/ARCHITECTURE.md}, Appendix~\ref{app:map}, Appendix~\ref{app:architecture}, and Appendix~\ref{app:realtime}.
...
Abuse controls are completed by operational safeguards: Helmet and CORS allowlists in \path{server/src/index.ts}; segmented rate limits for authentication, bots, slot, roulette, blackjack, hidden bets and multiplayer blackjack; idempotency middleware for sensitive mutations (\path{server/src/middleware/idempotency.middleware.ts}); and an admin surface that exposes cheater/report information without reusing player tokens. These measures do not replace a professional penetration test, but they provide a coherent defence-in-depth baseline for the academic prototype. Appendix~\ref{app:ops} links the operational controls.

\subsection{Testing and debugging}

Broad automated coverage on server and client; Playwright smoke tests under \path{client/e2e/}. Additional manual scenarios are documented under \path{Docs/POKER_SCENARIOS.md} and related files. Appendix~\ref{app:coverage} explains how test and coverage numbers are interpreted.

\subsection{Automated test inventory and CI (reference snapshot)}

Table~\ref{tab:tests} reflects a \textbf{May~2026} run (\texttt{cd server \&\& npm test}, \texttt{cd client \&\& npx vitest run}). Historical line counts also appear in \path{Docs/RAPPORT_TESTS.md}; refresh that file after large suite changes. Coverage definitions are in Appendix~\ref{app:coverage}.
...
\noindent
GitLab CI configuration at \path{.gitlab-ci.yml} encodes build and test gates for merge requests; the \texttt{docker-build} stage uses \textbf{Kaniko} for reproducible, rootless image builds pushed to the GitLab Container Registry. A consolidated validation narrative for bot flows and UI smoke checks appears in \path{Docs/rapport-validation.md}. CI operational detail is in Appendix~\ref{app:ops}.

\subsection{Performance and stability}

Redis-backed Socket.IO adapter, structured logging, Prometheus metrics endpoint, readiness endpoints, graceful shutdown hooks (\path{server/src/index.ts}). See Appendices~\ref{app:realtime} and~\ref{app:ops}.

\section{Technical Challenges}
\label{sec:challenges}

\textbf{Concurrency around shared tables:} poker and blackjack runtimes use locking strategies (see \path{server/src/poker/services/pokerTableLock.service.ts} and blackjack lock services) to prevent split-brain chip mutations. Related runtime details appear in Appendices~\ref{app:poker}, \ref{app:bj}, and~\ref{app:realtime}.

\textbf{Recovery after process restarts:} blackjack and tournament subsystems implement recovery services invoked at boot (\path{server/src/index.ts}); see Appendices~\ref{app:bj}, \ref{app:tournament}, and~\ref{app:realtime}.
...
For monetary integrity, chip-changing operations are handled server-side and backed by persistence patterns such as Prisma transactions, wallet ledger entries, idempotency keys and domain-specific settlement services. This pattern is visible in casino wallet modules, hidden-bet placement/resolution, tournament rewards and social loan flows. The common rule is that client-side balance displays are treated as cached UI, while the database and server-side services remain the source of truth. See Appendix~\ref{app:rewards}.

For the interface, the solution was to keep the first screen after login as a functional lobby rather than a marketing page. The lobby aggregates poker rooms, blackjack rooms, tournaments, friends, invitations and daily challenges; deeper pages remain dedicated to gameplay, profile, friends, leaderboard and admin operations. Internationalization is handled through i18next resources, and reusable UI components reduce duplication across poker, casino and social flows. UI route anchors are listed in Appendix~\ref{app:map}.

Finally, the project added validation and test coverage around these solutions. Schema tests protect authentication validation, integration tests cover waiting-room identity trust, and real-time gateway tests verify socket behavior with authenticated users and runtime snapshots. The remaining improvements would be a formal threat model and external penetration test before public deployment, broader end-to-end tests for the full lobby-to-game journey, and a polished observability dashboard for production operations; this is estimated at approximately two to three additional weeks depending on the desired depth. Appendix~\ref{app:coverage} gives the coverage definitions.
...
The repository does not contain signed-off, per-person effort logs, story point assignments, or validated textual attributions beyond the narratives above for Omar, Yigit and Azra. To satisfy examination requirements, the table below must still be \textbf{completed and countersigned by the team} for remaining members (replace placeholder cells).
...
Yigit & Debugging, QA, tournaments, responsive UI, Git/MR reviews & See narrative above; playtests on \path{client/src/pages/Game.tsx}, \path{client/src/features/tournament/}, mobile layouts and deployment regressions \\
\bottomrule
\end{tabular}
\caption{Individual contributions: expanded statements for Omar, Yigit and Azra appear as subsections above; other rows remain self-declared placeholders until completed.}\label{tab:contrib}
\end{table}
...
Quantum Bluff uses a \textbf{relational PostgreSQL database} accessed through Prisma. The authoritative schema is \path{server/prisma/schema.prisma}; migrations define the concrete SQL evolution. The central entity is \texttt{User}. Most gameplay, wallet, social, tournament and security tables reference it through foreign keys, which keeps identity, chip movements and audit history normalized instead of duplicating user information inside each feature.
...
\begin{longtable}{@{}p{3.2cm}p{4.6cm}p{6.1cm}@{}}
...
Hidden bets & \texttt{HiddenBetTicket}, \texttt{HiddenBetSelection}, \texttt{HiddenBetHandResolution} & A ticket belongs to a user and game/hand, with child selection rows and resolution records. \\
\bottomrule
\end{longtable}
