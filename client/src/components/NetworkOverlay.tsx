...
export function NetworkOverlay() {
  ...
  if (networkStatus === 'reconnected') {
    if (reconnectedDismissed) return null;
    return (
      <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-300" role="status" aria-live="polite">
        <div className="relative overflow-hidden rounded-2xl border border-emerald-500/50 bg-gradient-to-br from-emerald-950/98 via-teal-950/95 to-emerald-950/98 text-white shadow-[0_16px_48px_-8px_rgba(6,78,59,0.55),0_0_0_1px_rgba(52,211,153,0.18)_inset] backdrop-blur-xl">
          ...
          <div className="relative flex gap-3 px-4 py-3.5 pr-12">
            ...
            <div className="min-w-0 flex-1 pt-0.5">
              <p className="text-sm font-semibold tracking-tight text-emerald-50">{t('networkOverlay.reconnectedTitle')}</p>
              <p className="mt-1 text-xs leading-relaxed text-emerald-200/95">{t('networkOverlay.reconnectedHint')}</p>
            </div>
            <CheckCircle2 className="mt-1 h-5 w-5 shrink-0 text-emerald-300/95" aria-hidden />
          </div>
          <button type="button" onClick={() => setReconnectedDismissed(true)} className="absolute right-2 top-2 flex h-8 w-8 items-center justify-center rounded-lg text-emerald-200/90 transition-colors hover:bg-emerald-900/55 hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400/80" aria-label={t('networkOverlay.closeLabel')}>
            <X className="h-4 w-4" />
          </button>
        </div>
      </div>
  ...
  if (networkStatus !== 'offline' || offlineDismissed) return null;

  return (
    <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-300" role="alert" aria-live="assertive">
      <div className="relative overflow-hidden rounded-2xl border border-red-500/50 bg-gradient-to-br from-red-950/98 via-rose-950/95 to-red-950/98 text-white shadow-[0_16px_48px_-8px_rgba(127,29,29,0.65),0_0_0_1px_rgba(248,113,113,0.15)_inset] backdrop-blur-xl">
        ...
        <div className="relative flex gap-3 px-4 py-3.5 pr-12">
          ...
          <div className="min-w-0 flex-1 pt-0.5">
            <p className="text-sm font-semibold tracking-tight text-red-50">{t('networkOverlay.serverUnreachableTitle')}</p>
            <p className="mt-1 text-xs leading-relaxed text-red-200/90">
              {t('networkOverlay.serverUnreachableHint')}<br />
              {reconnectAttempts > 0 ? t('networkOverlay.reconnectWithAttempt', { count: reconnectAttempts }) : t('networkOverlay.reconnectAuto')}
            </p>
          </div>
