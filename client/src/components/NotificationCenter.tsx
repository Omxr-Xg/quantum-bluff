...
export function NotificationCenter() {
  ...
  const handleAcceptInvitation = async (inv: { invitationId: string; roomId: string }) => {
    try {
      ...
      const url = API_BASE ? `${API_BASE}/api/invitations/${inv.invitationId}/accept` : `/api/invitations/${inv.invitationId}/accept`;
  ...
  const handleRejectInvitation = async (inv: { invitationId: string }) => {
    try {
      const token = localStorage.getItem("token");
      const url = API_BASE ? `${API_BASE}/api/invitations/${inv.invitationId}/reject` : `/api/invitations/${inv.invitationId}/reject`;
      await fetch(url, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      });
    ...
    <div className="relative" ref={panelRef}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 max-h-[400px] overflow-y-auto bg-slate-800 border border-slate-600 rounded-xl shadow-2xl z-[300]">
          ...
          <div className="p-2">
            {totalCount === 0 ? (
              <p className="text-slate-400 text-sm py-6 text-center">{t("notifications.empty")}</p>
            ) : (
              <>
                    ...
                    {pendingInvitations.map((inv) => (
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-indigo-300/80 text-xs truncate">{inv.roomName}</p>
                    ...
                    {pendingFriendRequests.map((req) => (
                      ...
                      >
                        <p className="text-white text-sm truncate flex-1">
                          {t("toast.friendRequestFrom", { username: req.sender?.username ?? "?" })}
                        </p>
